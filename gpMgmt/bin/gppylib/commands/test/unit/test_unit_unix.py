...
class UnixCommandTestCase(GpTestCase):
    ...
    @patch('gppylib.commands.unix.get_cmd_version', return_value='rsync version 3.2.7')
    ...
    def test_compare_rsync_version(self, mock_parse_version, mock_get_cmd_version):
        ...
        self.subject.validate_rsync_version()

        mock_get_cmd_version.assert_called_once_with('rsync')
        mock_parse_version.assert_has_calls([call('3.2.7'), call('3.1.0')])


    @patch('gppylib.commands.unix.get_cmd_version', return_value='rsync version 2.6.9')
    @patch('gppylib.commands.unix.parse_version', side_effect=['2.6.9', '3.1.0'])
    def test_compare_rsync_version_exception(self, mock_parse_version, mock_get_cmd_version):
    ...
    @patch('gppylib.commands.unix.Command')
    def test_get_cmd_version(self, mock_command, mock_findCmdInPath):
        mock_command_instance = mock_command.return_value
        mock_command_instance.get_stdout.return_value = 'rsync version 3.2.7'
        version = self.subject.get_cmd_version('rsync')

        mock_findCmdInPath.assert_called_with('rsync')
        self.assertEqual(version, 'rsync version 3.2.7')


    @patch('gppylib.commands.unix.findCmdInPath', return_value=None)
    @patch('gppylib.commands.unix.Command')
    def test_get_cmd_version_exception(self, mock_command, mock_findCmdInPath):
        mock_command_instance = mock_command.return_value
        mock_command_instance.get_stdout.return_value = None
        with self.assertRaises(Exception) as ex:
            self.subject.get_cmd_version('nonexistent_command')

        self.assertEqual(str(ex.exception), "Unable to retrieve version information of command : nonexistent_command")
